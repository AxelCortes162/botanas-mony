...
const FloatingBar = ({ onOpenTransfer, onOpenCart }) => {
    ...
    <div className="pointer-events-none fixed inset-x-0 bottom-0 z-50 flex justify-center px-4 pb-[max(0.75rem,env(safe-area-inset-bottom))]">
      <div className="pointer-events-auto flex w-full max-w-lg gap-2">
        <button
          ...
          className="no-tap-highlight flex items-center justify-center gap-2 rounded-full bg-white px-5 py-3.5 font-extrabold text-ink shadow-lift transition hover:-translate-y-0.5 active:scale-95"
        >
          <span className="text-lg">💳</span>
          <span className="hidden text-sm sm:inline">Transferencia</span>
        </button>
        ...
        >
          <span className="text-lg">🛒</span>
          <span className="text-sm">{isEmpty ? 'Tu carrito' : `Ver pedido · ${money(subtotal)}`}</span>
